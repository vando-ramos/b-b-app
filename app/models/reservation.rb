class Reservation < ApplicationRecord
  ...
  def availability
    ...
    existing_reservations = room.reservations.where.not(id: id).where.not(status: 'canceled')
    overlapping_reservations = existing_reservations.where('(start_date <= ? AND end_date >= ?) OR
    ...
    if num_guests > room.maximum_guests || num_guests <= 0
      errors.add(:guests, 'Number of guests exceeds room capacity or is zero')
    end
  end
end
